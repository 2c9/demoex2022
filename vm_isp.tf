resource "vsphere_virtual_machine" "isp" {
    ...
    name             = format("${var.prefix}ISP-${var.index}")
    ...
    network_interface {
        network_id   = data.vsphere_network.network_isp_cli.id
        adapter_type = data.vsphere_virtual_machine.debian_template.network_interface_types[0]
    }

    network_interface {
        network_id   = data.vsphere_network.network_isp_rtrl.id
        adapter_type = data.vsphere_virtual_machine.debian_template.network_interface_types[0]
    }
    ...
    tags = ["${data.vsphere_tag.debian11.id}", "${data.vsphere_tag.isp.id}"]
}